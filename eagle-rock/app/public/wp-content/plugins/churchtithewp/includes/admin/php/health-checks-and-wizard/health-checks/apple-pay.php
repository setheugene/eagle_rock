...
function church_tithe_wp_heath_check_apple_pay( $health_checks ) {
	...
	if ( 'connected' === $apple_pay_connected_domain_status && ! church_tithe_wp_is_site_localhost() ) {
		$is_healthy = true;
	} else {
		$is_healthy = false;
	}
	...
	if ( $is_healthy ) {
		...
		if ( ! $apple_verification_file_exists ) {
			...
			$unhealthy_array = array(
				'instruction'        => __( 'Apple Pay is not hooked up!.', 'church-tithe-wp' ),
				'fix_it_button_text' => __( 'Let\'s fix it!', 'church-tithe-wp' ),
				'component_data'     => array(
					...
					'strings'                             => array(
						...
						'description' => __( 'The Apple Verification file could not be created.', 'church-tithe-wp' ),
					),
					'stripe_connect_url'                  => church_tithe_wp_get_stripe_connect_button_url( 'live' ),
				),
			);
