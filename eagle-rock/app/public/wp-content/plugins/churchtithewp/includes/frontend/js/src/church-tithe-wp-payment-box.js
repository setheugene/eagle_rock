...
window.Church_Tithe_WP_Payment_Box = class Church_Tithe_WP_Payment_Box extends React.Component {
	...
	maybe_render_close_button() {
		...
		return (
			<div className="church-tithe-wp-close-btn" aria-label="Close" onClick={ this.props.main_component.handle_visual_state_change_click_event.bind( this, {}, {} ) }><img src={ this.props.main_component.state.unique_settings.close_button_url } /></div>
		);
	}
	...
	render() {
		...
		if ( 'none' == this.state.current_visual_state ) {
			...
			return (
				<StripeProvider apiKey={this.props.main_component.state.unique_settings.stripe_api_key}>
...
class Church_Tithe_WP_Card_Form extends React.Component {
	...
	country_supports_payment_request_button() {
		var country_code = this.props.main_component.state.unique_settings.stripe_account_country_code;
	...
	check_for_payment_request_availability() {
		...
		if ( this.props.stripe && ! this.state.stripe_is_set ) {
			...
			var paymentRequest = this.props.stripe.paymentRequest({
				country: this.props.main_component.state.unique_settings.stripe_account_country_code,
	...
	get_visual_amount() {
		...
		var locale = this.props.main_component.state.unique_settings.locale.replace("_", "-");
	...
	do_ajax_stripe_transaction() {
		...
		postData.append('church_tithe_wp_payment_intent_nonce', this_component.props.main_component.state.frontend_nonces.payment_intent_nonce);
					...
					<div className={ 'church-tithe-wp-tithe-amount-input-container' }>
						<label>
							<input
								...
								lang={ this.props.main_component.state.unique_settings.locale.replace("_", "-") }
							/>
						</label>
					</div>
	...
	render_complete_payment_button_field() {
		...
		return (
			<div>
			<button type="submit" disabled={ this.get_disabled_status( [ 'credit_card' ] ) } className={ 'church-tithe-wp-pay-button' + button_class }>{ button_text }</button>
			</div>
		)
	}
