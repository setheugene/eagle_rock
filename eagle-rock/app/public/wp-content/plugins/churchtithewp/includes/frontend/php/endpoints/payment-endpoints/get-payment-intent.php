...
function church_tithe_wp_get_payment_intent_handler() {
	...
	// Verify the nonce.
	if ( ! isset( $_POST['church_tithe_wp_payment_intent_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['church_tithe_wp_payment_intent_nonce'] ) ), 'church_tithe_wp_payment_intent_nonce' ) ) {
		return array(
			'success'    => false,
		...
		$s = new Church_Tithe_WP_Stripe(
			array(
				...
				'fields' => array(
					...
					'source'         => $church_tithe_wp_stripe_payment_method_id,
				),
			)
		);
	...
	if ( isset( $payment_method_attached_to_customer['error'] ) ) {
		...
		// If we got an actual error from Stripe...
		if ( isset( $payment_method_attached_to_customer['error']['code'] ) ) {
			$details    = $payment_method_attached_to_customer['error']['message'];
			$error_code = $payment_method_attached_to_customer['error']['code'];
		} else {
			...
			$body      = __( 'Please email support@church_tithe_wpchurchtithewp.com with the following information for assistance.', 'church-tithe-wp' ) . ' ' . wp_json_encode( $payment_method_attached_to_customer ) . "\n" . __( 'Data in request:', 'church-tithe-wp' ) . "\n" . wp_json_encode( $s->fields );
	...
	if ( isset( $stripe_customer['error'] ) ) {
		...
		// translators: The url of this website.
		$subject   = sprintf( __( 'A Payment has failed on %s.', 'church-tithe-wp' ), get_bloginfo( 'url' ) );
		$body      = __( 'Please email support@church_tithe_wpchurchtithewp.com with the following information for assistance.', 'church-tithe-wp' ) . ' ' . wp_json_encode( $stripe_customer['error'] ) . "\n" . __( 'Data in request:', 'church-tithe-wp' ) . "\n" . wp_json_encode( $s->fields );
		$mail_sent = wp_mail( $admin_email, $subject, $body );
	...
	if ( ! church_tithe_wp_fee_threshold_reached() ) {
		$payment_intent_args['fields']['application_fee_amount'] = absint( $church_tithe_wp_amount * .01 );
	}

	// Send a call to Stripe to generate a PaymentIntent.
	$s = new Church_Tithe_WP_Stripe( $payment_intent_args );
	...
	return array(
		'success'          => true,
		'session_id'       => church_tithe_wp_create_payment_session( $user_id ),
		'user_id'          => $user_id,
