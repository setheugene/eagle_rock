...
function church_tithe_wp_create_stripe_subscription_from_arrangement( $arrangement, $transaction, $charge ) {
	...
	$s = new Church_Tithe_WP_Stripe(
		array(
			...
			'fields'          => array(
				...
				// Here, we offset the first charge in the subscription because the original PaymentIntent (this webhook) is considered to be the first charge.
				// It is done this way to offset all of the subscription logic to the webhook, thus speeding up the original purchase time immensely.
	...
	$arrangement_data = array(
		'gateway_subscription_id' => isset( $subscription['id'] ) ? $subscription['id'] : null,
