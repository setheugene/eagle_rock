 ...
 * @return      bool
 */
function church_tithe_wp_payment_session_valid( $user_id, $session_id ) {
	...
	$saved_session = get_transient( 'ctwp_payment_session_' . $user_id );
 ...
 * @access      public
 * @since       1.0.0.
 * @param       string $user_id The ID of the user to whom this session will belong.
 * @return      string
 */
function church_tithe_wp_create_payment_session( $user_id ) {
	...
	set_transient( 'ctwp_payment_session_' . $user_id, wp_hash_password( $session_id ), HOUR_IN_SECONDS );
